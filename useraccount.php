...
$review_str = "select review_content, review_id, review_rating, game_name, game_id from reviews join games on reviews.review_game_id=games.game_id where reviews.review_user_id=" . $user_id;
	...
	<div class="container wrapper">
		...
		<div class="row">
			<div class="col-xs-8 col-xs-offset-2">
				...
				<p class="lead">Welcome to your user dashboard! Here you can edit your information, see favorite games, and your own reviews!</p>
			</div>
		</div>
		<div class="row">
			<div class="col-xs-8 col-xs-offset-2">
			...
			<div class="col-xs-6">
				...
				if (isset($items)) {
					...
					foreach ($items_count_value as $id => $qty) {
						//select statement
						$query_str = "SELECT * FROM games WHERE game_id=" . $id;
						//execute the query
						$result = $conn->query($query_str);
						$result_row = $result->fetch_assoc();

						$id = $result_row['game_id'];
						$img = $result_row['game_img'];
						$title = "<a href='gamedetails.php?id= " . $result_row['game_id'] . "'>" . $result_row['game_name'] . "</a>";
					...
					echo "<p class='lead text-center'>You don't have any favorite games!</p>";
				}
				?>
			</div>
			<div class="col-xs-6">
				...
				<?php
					while ($review_row = $review_result->fetch_assoc()) { ?>
						...
						<div class="panel panel-default">
							<div class="panel-heading game-list">
								<h3 class="panel-title">
									<a href="gamedetails.php?id=<?= $review_row['game_id'] ?>"><?= $review_row['game_name'] ?></a>
								</h3>
							</div>
